...
function getAllTokenAddresses() {
    ...
    var joined = DEFAULT_TOKEN_ADDRESSES.concat(customTokens || []);
    return joined.filter(function(item, index) {
        return joined.indexOf(item) === index;
...
function saveTokenMeta(meta) {
    ...
    var key = web3util.utils.toChecksumAddress(meta.address);
    if(cache[key]) {
        return cache;
    }
    cache[key] = {
    ...
    writeJsonStorage("tokenMetaCache", cache);
    return cache;
}
...
function getTokenMetaByAddress(address) {
    ...
    var cache = getStoredTokenMeta();
    return cache[web3util.utils.toChecksumAddress(address)] || null;
}

function ensureDefaultTokenCache(done) {
    var addresses = getAllTokenAddresses();
    var pending = addresses.map(function(address) {
        if (getTokenMetaByAddress(address)) {
            return Promise.resolve();
        }
        return fetchTokenMeta(address).then(function(meta) {
            saveTokenMeta(meta);
        }).catch(function() {
            return Promise.resolve();
        });
...
async function web3Call(web3, to, method) {
    var args = [];
