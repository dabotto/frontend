var IndexController = function(view) {
    ...
    context.refreshBalance = async function refreshBalance() {
        ...
        try {
            ...
            if(tokenAddress === voidEthereumAddress) {
                allowance = balance = await web3.eth.getBalance(session.address);
            } else {
                balance = abi.decode(['uint256'], await call(tokenAddress, 'balanceOf(address)', session.address))[0].toString();
                allowance = abi.decode(['uint256'], await call(tokenAddress, 'allowance(address,address)', session.address, window.context.managerAddress))[0].toString();
            }
    ...
    context.addLiquidity = async function addLiquidity(address, value, subject) {
        ...
        if(subject.toLowerCase() !== voidEthereumAddress && subject.toLowerCase() !== session.address.toLowerCase() && !context.view.state.isOwner) {
            if(!await manager.methods.referrals(session.address).call()) throw new Error("Only the owner or an enabled referral can deposit for another address.");
        }
    ...
    context.manageReferrals = function(toAdd, toRemove) {
        ...
        return prepareAndSendTx(context.getManager().methods.manageReferrals(additions, removals), { returnReceipt: true, throwError: true });
    }
