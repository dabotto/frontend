var IndexController = function(view) {
    ...
    context.refreshBalance = async function refreshBalance() {
        if(!window.web3?.currentProvider || !context?.view?.state?.walletAddress || context?.view?.state?.mode === 'none' || !context.view.state[context?.view?.state?.mode + "TokenAddress"]) {
    ...
    context.rebalance = async function rebalance() {
        ...
        while(true) {
            try {
                await context.manager.methods.pools(idx).estimateGas({
                    from : context.view.state.walletAddress,
                    gas : 16700000
                });
                return await prepareAndSendTx(context.manager.methods.pools(idx));
            } catch(e) {
    ...
    async function prepareAndSendTx(method, options = {}) {
        ...
        options.value !== '0' && (tx.value = web3utils.numberToHex(options.value));
        tx = {
            ...tx,
    ...
    context.init = async function init() {
        ...
        try {
            ...
            var futureReward = claimRewardResult.participationPercentage.toString() === "0" ? 0 : parseInt(claimRewardResult._nextRebalanceEvent) - parseInt((await web3.eth.getBlock('latest')).timestamp);
            if(futureReward > 0) {
            ...
            for(var item of syncedPools) {
                ...
                var position = {
                    ...
                    statusResult : item.rebalanceNeeded ? 2 : item.surplus0 !== '0' || item.surplus1 !== '0' ? 1 : 0,
